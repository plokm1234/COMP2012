...
BinarySearchTree* Manager::getSection(string sectionType)
{
    ...
    return this->managerRegistry[sectionType];
}
...
void Manager::registerSection(string sectionType, BinarySearchTree* sectionInstance)
{
    ...
    this->managerRegistry.insert(pair<string, BinarySearchTree*>(sectionType, sectionInstance));
}
...
void Manager::deleteSection(string sectionType)
{
    ...
    delete this->managerRegistry[sectionType];
    this->managerRegistry.erase(sectionType);
}
...
void Manager::printStatus()
{
    ...
    if (managerRegistry.size() != 0){
        ...
        std::for_each(this->managerRegistry.begin(), this->managerRegistry.end(), [](const std::pair<string, BinarySearchTree*>& element){ cout << " " << element.first; });
    }   
}
...
Manager::~Manager()
{
    ...
    // Hint: It can be done using a single line of code
    // Hint2: The type of the elements in a map<A,B> is pair<A,B>
    std::for_each(this->managerRegistry.begin(), this->managerRegistry.end(), []( std::pair<string, BinarySearchTree*> element){ delete element.second; });
}
